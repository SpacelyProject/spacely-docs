...
- For all analog/bias pins, the anticipated voltage and current to source/sink.
...
We want to determine whether the gain of the ASIC matches design specifications.
We will drive the ASIC VIN pin with a sine wave generated by an Agilent 33250A waveform generator and measure its VOUT pin with an oscilloscope. The input sine wave will have a frequency in the range 1 kHz to 100 MHz and an amplitude of 0.1 V. We will use Spacely to sweep the frequency of the pulse generator, capture waveforms from the Oscilloscope, and calculate the peak-to-peak amplitude of the output signal. We will use this data to plot gain versus frequency and compare the result to the design specification.

In this example:
- *Input:* Sine wave from A33250A waveform generator, frequency 1kHz to 100 MHz, amplitude 0.1 V 
- *Output:* Oscilloscope
- *Measurement procedure:* Sweep frequency and calculate peak-to-peak amplitude with Spacely, plot gain vs frequency.

### Bad Example Objective
We will measure the SNR of the ADC.

In this example, many questions remain: What is the input signal? What frequency range is important? How many samples will you take? Etc, etc...

The test setup required may be wildly different depending on the answers to these questions.
...
In most cases, you will need to create a printed circuit board to test your ASIC, and you may need to create other hardware. Your Test Setup defined in the previous section should help with this. The following questions can guide your design. 

Your ASIC Printed Circuit Board:
- Does the board need to go into a cryostat, and if so, how will it be thermalized to the cold plate? Do you have room on the circuit board to mount an RTD (resistive thermal device)? 
- If you plan to use Spacely-Caribou, does your board have a SEARAY connector to mate to the CaR board?


Other hardware:
- If your chip is tested at cryo, consider whether you need to design a flange PCB (for electrical signals) or alternative flanges (i.e. for optical signals). Remember that the SEARAY connector is too large to fit through existing flanges.
- Do you need a 3D-printed stand or fixture to hold the PCB either during test or during wirebonding? 
